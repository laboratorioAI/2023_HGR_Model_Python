...
# Directorio que contiene los archivosz JSON
directorio = 'D:\Marco Salazar Tesis\CodigoTesis\DynamicData'
...
df_completo["Quat_Spectrograms"] = df_completo["Quat_Spectrograms"].apply(np.array)

df_completo["Quat_Spectrograms"] = df_completo["Quat_Spectrograms"].apply(lambda x: scaler.fit_transform(x.reshape(-1, 1)).flatten().reshape((13,24,4)))

# Restaura la forma original de los datos
df_completo["Quat_Spectrograms"] = df_completo["Quat_Spectrograms"].apply(lambda x: x.reshape(13, 24, 4))

# Guarda el scaler en un archivo binario
with open('scalerDynamicTestingAll.pkl', 'wb') as f:
...
x_train = train_df['Quat_Spectrograms'].values
y_train = train_df['Gesture'].values
x_val = val_df['Quat_Spectrograms'].values
...
y_val_pred = modelD.predict(x_val)
...
fig.savefig("matriz_confusionD.png")
...
model_filename = f"{folder_name}/modeloD_{current_datetime}.h5"
